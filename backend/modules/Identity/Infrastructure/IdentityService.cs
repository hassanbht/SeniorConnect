...
public sealed class IdentityService : IIdentityService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // In dev, log the verification link; in prod, send via email
        var verificationLink = $"https://seniorconnect.at/verify-email?token={rawToken}";
        var emailBody = $"Please verify your email by clicking: {verificationLink} (valid 24 hours)";
        await _otpService.GenerateAndSendEmailOtpAsync(normalizedEmail, OtpPurpose.Registration, ipAddress, request.PreferredLocale, cancellationToken);
