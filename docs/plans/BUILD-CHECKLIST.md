...
[x] P5-04 — Recurring events (RRULE, lazy occurrences) · L — implemented
    **2026-09 fix (this session):** "lazy occurrences" was previously just
    two fields (`RecurrenceFrequency`/`RecurrenceUntilUtc`) with no
    expansion or per-occurrence state at all — cancelling a recurring event
    always cancelled the whole series, there was no way to cancel a single
    date. Fixed: `CommunityEvent.ComputeOccurrenceStartsUtc()` expands
    occurrences on read (never persisted, bounded to 52), a new
    `CommunityEventOccurrenceCancellation` row is the only thing that
    persists per-occurrence, and `POST /events/{id}:cancel-occurrence`
    validates the date is a real occurrence before recording it.
    ✓ Test: PASSING — `EventOccurrenceCancellationTests` (5 tests: series
    stays uncancelled, duplicate-cancel conflicts, non-occurrence date and
    non-recurring event both rejected, non-staff forbidden)
[x] P5-05 — Registration, capacity, waiting list · M — implemented (EventRegistration)
[x] P5-06 — Distance + interest discovery        · M — implemented.
    `GET /api/v1/discovery/nearby-community-events` (Geography module):
    reuses the existing `AustrianAdministrativeUnit` postal-code→lat/lng
    table (no new columns on `CommunityEvent`) to resolve each event's
    postal code, Haversine-filters by radius, and optionally restricts to
    the caller's saved `Interest.Code`s matched against the event
    `Category` (`matchMyInterests=true`; an empty/no interest set means "no
    filter", never "show nothing"). Cross-module read goes through a new
    `Community.Contracts.ICommunityDiscoveryReader`, mirroring the existing
    `HelpRequests.Contracts` pattern — never touches Community's
    Domain/Application/Infrastructure directly. Wired into
    `community_feed_screen.dart` as a "Nearby" filter chip (Riverpod
    `CommunityFeedNotifier.toggleNearbyMode()`), reusing
    `DiscoveryRepository.getMyLocation()` from the existing P2-41 discovery
    feature; distance shown in km, category chips hidden while active
    since the server doesn't filter by category+distance together.
    ✓ Test: PASSING — `ProximityServiceTests` (2 new: radius filtering,
    interest filtering excludes non-matching category)
[x] P5-07 — "Meine Termine" (GetMyScheduleAsync) · M — implemented.
    **2026-09 fix (this session):** the mobile screen called the wrong path
    (`/api/v1/community/me/schedule` vs. the real `/my-schedule`) and the
    cancel button called `DELETE .../register` vs. the real
    `POST .../register:cancel` — both 404'd every time, silently falling
    back to hardcoded demo data / a fake local-only cancel. Fixed in
    `my_appointments_notifier.dart`.
...
[x] P5-09 — Contextual conversation threads (MessageThread/ThreadMessage) · M
    **2026-09 fix (this session) — real privacy bug:** `GetMessagesAsync`/
    `PostMessageAsync`/`GetOrCreateThreadAsync` had zero authorization —
    any authenticated user who guessed or observed a thread/context id
    could read and post into a `PrivateInvitation` or `OrganizationMembers`
    group's thread. The repo's own `ThreadPrivacyTests.cs` already encoded
    the intended rule (non-Public groups require active membership; Event
    threads stay open by design) and was failing 2/39 before the fix. Fixed
    via `CommunityService.RequireThreadAccessAsync`, wired into all five
    thread-touching methods. All 44 Community tests pass now.
[x] P5-mobile — Audit mobile community_feed_screen.dart for any
    individual-facing "create group" affordance; remove if present · S
    **2026-09 audit (this session):** confirmed clean — grepped the whole
    mobile app for `CommunityGroup`/group-create screens/routes: zero hits.
    The `community.create_group`/`join`/`members`/`groups` translation keys
    exist but are unused dead strings, not a UI affordance. Nothing to remove.
```
...
[x] An individual user has no UI path either — audited this session, see P5-mobile
[x] Cancelling one occurrence does not cancel the series — fixed and tested
    this session, see P5-04
[x] Private/context-bound thread content invisible to non-participants —
    fixed this session (real gap found via pre-existing failing tests),
    see P5-09; verified via ThreadPrivacyTests (5 tests, all passing)
[ ] Joining an event takes ≤ 3 taps from the Senior Mode home screen — traced
    the route statically (home action → feed → event detail → join button =
    3 taps) but not exercised in a running app this session
