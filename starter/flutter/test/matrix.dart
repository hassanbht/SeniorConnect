...
import 'package:SeniorConnect/core/design_system/app_theme.dart';
import 'package:SeniorConnect/core/design_system/app_tokens.dart';
      ...
      for (final s in seniorModes)
        for (final t in textScales)
          for (final l in locales)
            MatrixCase(brightness: b, seniorMode: s, textScale: t, locale: l),
  ];
}
...
Future<void> expectNoOverflow(WidgetTester tester) async {
  ...
  expect(
    ...
    isNull,
    reason:
        'Layout overflowed. A screen must survive text scale 2.0 — see '
...
Future<void> expectSeniorTouchTargets(WidgetTester tester, MatrixCase c) async {
  ...
  for (final element in find.byType(InkWell).evaluate()) {
    ...
    expect(
      ...
      isTrue,
      reason:
          'Senior Mode requires ${min}dp touch targets, found $size. '
