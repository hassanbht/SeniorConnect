...
class CoordinatorHoursQueueNotifier extends _ {
  ...
  Future<void> loadQueue(ApiClient apiClient, String orgId) async {
    ...
    try {
      final resp = await apiClient.get<dynamic>('/api/v1/coordinator/attention/unconfirmed-hours?organizationId=');
      state = state.copyWith(activities: resp is List ? resp : [], isLoading: false);
    } catch (_) { state = state.copyWith(isLoading: false, errorMessage: 'errors.generic'); }
  }
  Future<void> confirmActivity(String activityId, ApiClient apiClient, String orgId) async {
    try {
  ...
  Future<void> batchConfirmAll(ApiClient apiClient, String orgId) async {
    ...
    try {
      for (final a in state.activities) {
        ...
        if (id != null) await apiClient.post<dynamic>('/api/v1/activities/', data: {});
      }
      await loadQueue(apiClient, orgId);
    } catch (_) { state = state.copyWith(errorMessage: 'errors.generic'); }
    finally { state = state.copyWith(isBatchProcessing: false); }
  }
}
