...
class CoordinatorBulkEntryNotifier extends _ {
  ...
  Future<void> _loadRef(ApiClient apiClient, String orgId) async {
    ...
    try {
      final vols = await apiClient.get<dynamic>(
        '/api/v1/coordinator/volunteers?organizationId=',
      );
      final cats = await apiClient.get<dynamic>(
      ...
      state = state.copyWith(
        ...
        categories: c,
        isLoading: false,
        selectedVolunteerId: v.isNotEmpty
            ? (v.first as Map<String, dynamic>)['userId'] as String?
            : null,
        selectedCategoryId: c.isNotEmpty
            ? (c.first as Map<String, dynamic>)['id'] as String?
            : null,
      );
    } catch (_) {
