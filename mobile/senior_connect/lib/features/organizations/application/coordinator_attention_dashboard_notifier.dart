...
class CoordinatorAttentionDashboardNotifier extends _ {
  @override
  CoordinatorAttentionDashboardState build(ApiClient apiClient, String organizationId) {
  ...
  Future<void> _load(ApiClient apiClient, String orgId) async {
    ...
    try {
      final triage = await apiClient.get<dynamic>('/api/v1/coordinator/triage?organizationId=');
      ...
      try {
        final e = await apiClient.get<dynamic>('/api/v1/coordinator/attention/expiring-verifications');
        if (e is List) exp = e;
      } catch (_) {}
      try {
        final s = await apiClient.get<dynamic>('/api/v1/coordinator/attention/silent-volunteers?organizationId=');
        if (s is List) silent = s;
      } catch (_) {}
      state = state.copyWith(isLoading: false, unconfirmedCount: unconf, disputedCount: disp,
        pendingAppsCount: pend, expiringVerifications: exp, silentVolunteers: silent);
  ...
  Future<void> sendReminders(String orgId, ApiClient apiClient) async {
    ...
    try { await apiClient.post<dynamic>('/api/v1/coordinator/volunteers/send-reactivation-reminders', data: {'organizationId': orgId}); }
    catch (_) {}
