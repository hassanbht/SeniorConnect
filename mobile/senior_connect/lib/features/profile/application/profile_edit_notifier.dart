...
class ProfileEditNotifier extends _$ProfileEditNotifier {
  ...
  Future<void> _loadInterests(InterestsRepository interestsRepo) async {
    try {
      ...
      state = state.copyWith(
        ...
        selectedInterestIds:
            (results[1] as List<InterestOption>).map((i) => i.id).toSet(),
      );
    } catch (_) {
  ...
  void toggleInterest(String id, bool selected) {
    ...
    if (selected) { updated.add(id); } else { updated.remove(id); }
  ...
  Future<void> onBundeslandChanged(
    ...
    if (code == null) {
      state = state.copyWith(
        selectedBundesland: null, selectedBezirk: null,
        selectedGemeinde: null, bezirke: [], gemeinden: []);
      return;
    }
    state = state.copyWith(
      selectedBundesland: code, selectedBezirk: null,
      selectedGemeinde: null, bezirke: [], gemeinden: []);
    try {
  ...
  Future<void> onBezirkChanged(
    ...
    if (code == null) {
      state = state.copyWith(selectedBezirk: null, selectedGemeinde: null, gemeinden: []);
      return;
    }
    state = state.copyWith(selectedBezirk: code, selectedGemeinde: null, gemeinden: []);
    try {
  ...
  Future<void> geocodeAddress(
    ...
    try {
      final result = await geographyRepo.geocodeAddress(fullAddress, persistToProfile: true);
      state = state.copyWith(latitude: result.latitude, longitude: result.longitude);
    } catch (_) { state = state.copyWith(errorKey: 'errors.generic'); }
    finally { state = state.copyWith(isGeocoding: false); }
  }

  void setPhoneVerified() => state = state.copyWith(phoneVerified: true);
  void setVerifyingPhone(bool v) => state = state.copyWith(isVerifyingPhone: v, errorKey: null);
  void setError(String key) => state = state.copyWith(errorKey: key);
  void clearError() => state = state.copyWith(errorKey: null);
  void setLocale(String locale) => state = state.copyWith(preferredLocale: locale);
  ...
  Future<void> save(
    ...
    finally { state = state.copyWith(isSaving: false); }
  }
}
