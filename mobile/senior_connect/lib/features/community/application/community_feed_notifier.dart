  ...
  Future<void> _loadEvents(ApiClient? apiClient, String? category) async {
    ...
    try {
      if (apiClient != null) {
        ...
        final resp = await apiClient.get<List<dynamic>>('/api/v1/community/events');
        state = state.copyWith(events: resp.map((e) => Map<String,dynamic>.from(e as Map)).toList(), isLoading: false);
        return;
      }
    } catch (_) {}
    state = state.copyWith(isLoading: false, events: const [
      {'id':'ev-1','title':'Senioren-Schachtreff','category':'sports'},
      {'id':'ev-2','title':'Gemeinsames Kaffeetrinken','category':'general'},
    ]);
  }
