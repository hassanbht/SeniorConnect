...
class MyAppointmentsNotifier extends _ {
  ...
  Future<void> _load(ApiClient apiClient) async {
    ...
    try {
      final resp = await apiClient.get<List<dynamic>>('/api/v1/community/me/schedule');
      state = state.copyWith(isLoading: false,
        appointments: resp.map((e) => Map<String,dynamic>.from(e as Map)).toList());
    } catch (_) {
      state = MyAppointmentsState(isLoading: false, appointments: [
        {'eventId':'ev-1','title':'Senioren-Schachtreff','myStatus':'Going','isCancelled':false}
      ]);
    }
  }
  Future<void> cancelRegistration(String eventId, ApiClient apiClient) async {
    try { await apiClient.delete<dynamic>('/api/v1/community/events//register'); await _load(apiClient); }
    catch (_) { state = state.copyWith(error: 'errors.generic'); }
  }
}
