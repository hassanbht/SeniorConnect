...
class SafeguardingConcernNotifier extends _ {
  ...
  Future<void> submit(String summary, String subjectUserId, ApiClient? apiClient) async {
    if (summary.trim().isEmpty) return;
    state = state.copyWith(isSubmitting: true, errorKey: null);
    try {
      if (apiClient != null) {
        await apiClient.post<dynamic>('/api/v1/safeguarding/concerns', data: {
          'subjectUserId': subjectUserId, 'summary': summary.trim(),
          'category': state.selectedCategory, 'severity': 'Medium',
        });
      }
