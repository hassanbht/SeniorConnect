...
class SeniorRequestFlowNotifier extends _$SeniorRequestFlowNotifier {
  ...
  Future<void> _loadCategories(
    ...
    try {
      final raw = initialCategories ??
          (await apiClient.get<List<dynamic>>('/api/v1/activities/categories'))
  ...
  void selectCategory(HelpCategoryItem category) {
    ...
    state = state.copyWith(
      ...
      currentStep: blocked ? RequestStep.blockedReferral : RequestStep.selectTiming,
    );
  }

  void selectTiming(String timing) => state = state.copyWith(
    selectedTiming: timing, currentStep: RequestStep.addDetails);

  void goToReview() => state = state.copyWith(currentStep: RequestStep.reviewAndConfirm);
  ...
  Future<void> submit(String details, ApiClient apiClient) async {
    ...
    try {
      ...
      state = state.copyWith(isSubmitting: false, errorMessage: 'errors.generic');
    }
  }
}
