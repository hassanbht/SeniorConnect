...
class MyRequestStatusNotifier extends _ {
  ...
  Future<void> _load(ApiClient apiClient, String? requestId) async {
    if (requestId == null) { state = const MyRequestStatusState(isLoading: false, loadError: 'errors.generic'); return; }
    ...
    try {
      ...
      state = MyRequestStatusState(isLoading: false, details: RequestStatusDetails(
        status: m['status'] as String? ?? 'Pending',
        categoryNameKey: catKey,
        scheduledStartUtc: m['scheduledStartUtc'] != null ? DateTime.tryParse(m['scheduledStartUtc'] as String) : null,
        notes: m['notes'] as String?,
        volunteerDisplayName: m['volunteerDisplayName'] as String?,
        volunteerPhone: m['volunteerPhone'] as String?,
      ));
    } catch (_) {
      state = const MyRequestStatusState(isLoading: false, loadError: 'errors.generic');
    }
  }
}
