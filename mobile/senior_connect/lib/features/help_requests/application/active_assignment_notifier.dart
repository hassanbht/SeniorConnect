...
class AssignmentDetails with _ {
  const factory AssignmentDetails({
    ...
    String? address, String? notes, String? seniorDisplayName, String? seniorPhone,
...
class ActiveAssignmentState with _ {
  const factory ActiveAssignmentState({
    ...
    String? loadError, String? actionError, AssignmentDetails? details,
...
class ActiveAssignmentNotifier extends _ {
  ...
  Future<void> _load(ApiClient apiClient, String? id) async {
    if (id == null) { state = const ActiveAssignmentState(isLoading: false, loadError: 'errors.generic'); return; }
    state = const ActiveAssignmentState(isLoading: true);
    try {
      final m = await apiClient.get<Map<String, dynamic>>('/api/v1/help-requests/');
      final cats = await apiClient.get<List<dynamic>>('/api/v1/activities/categories');
      var catKey = 'help.category.shopping';
      final catId = m['categoryId'] as String?;
      if (catId != null) {
        final match = cats.cast<Map<String, dynamic>>().where((c) => c['id'] == catId).firstOrNull;
        if (match != null) catKey = 'help.category.';
      }
      state = ActiveAssignmentState(isLoading: false,
        isCheckedIn: m['status'] == 'CheckedIn', isCompleted: m['status'] == 'Completed',
        details: AssignmentDetails(categoryNameKey: catKey, address: m['address'] as String?,
          notes: m['notes'] as String?, seniorDisplayName: m['seniorDisplayName'] as String?,
          seniorPhone: m['seniorPhone'] as String?, rowVersion: m['rowVersion'] as int? ?? 0));
    } catch (_) {
      state = const ActiveAssignmentState(isLoading: false, loadError: 'errors.generic');
    }
  }
  Future<void> checkIn(String id, ApiClient apiClient) async {
    ...
    } catch (_) { state = state.copyWith(actionError: 'errors.generic', isActionInProgress: false); }
  }
