...
import 'package:senior_connect/shared/widgets/app_states.dart';
import 'package:senior_connect/shared/widgets/app_status.dart';
...
void main() {
  group('AppButton', () {
    testAcrossMatrix(
      'renders and passes accessibility guidelines',
      (tester, c) async {
    ...
    testWidgets('does not fire twice on a rapid double tap', (tester) async {
      ...
      expect(
        count,
        1,
        reason: 'A senior tapping twice must not submit twice.',
      );
      await tester.pumpAndSettle();
    });
    ...
    testWidgets('destructive variant requires confirmation', (tester) async {
      ...
      expect(find.text('Möchten Sie den Termin wirklich absagen?'), findsOneWidget);
    });
  });

  group('AppStatusChip', () {
    testAcrossMatrix(
      'status is carried by icon and word, not only colour',
      (tester, c) async {
        await tester.pumpWidget(
          wrapForTest(
            const Padding(
              ...
              child: Wrap(
                ...
                runSpacing: 8,
                children: [
                  AppStatusChip(label: 'Wird gesucht', tone: AppStatusTone.pending),
  ...
  group('AppEmptyState', () {
    testAcrossMatrix(
      'renders a message and an action, never "Keine Daten"',
      (tester, c) async {
        await tester.pumpWidget(
          wrapForTest(
            AppEmptyState(
  ...
  group('AppErrorView', () {
    testAcrossMatrix(
      'a 403 renders what is missing as a checklist',
      (tester, c) async {
        await tester.pumpWidget(
          wrapForTest(
            AppErrorView(
